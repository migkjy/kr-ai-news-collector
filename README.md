...
```
GeekNews: 전체 30 · 매칭 12 · 신규 12
AI타임스: 전체 50 · 매칭 41 · 신규 41
ZDNet Korea: 전체 20 · 매칭 7 · 신규 7
...
신규 78건 → output/digest-2026-08-25.md
```
...
전부 공개 RSS이고, robots.txt에서 일반 크롤러(`*`)에 허용된 곳만 실었습니다(2026-08 확인). 피드 간 1초 간격으로 요청합니다.
...
**Windows** — 작업 스케줄러에서 "프로그램 시작" 동작으로 `node`, 인수 `collect.mjs`, 시작 위치를 클론 폴더로 지정.
...
이미 한 번 수집한 기사는 다시 잡지 않습니다. 처음부터 다시 모으려면 `data/seen-urls.json`을 지우세요.
